...
import { DEFAULT_SEARCH_ASSEMBLY } from '../domain/contextAssembly.js';
...
import { type ChatWorkflowResult, runChatStream } from './ChatWorkflow.js';
...
describe('ChatWorkflow', () => {
  ...
  it('B1_timeout_stops_stream', async () => {
    ...
    const gen = runChatStream(
      { store, embedder, chat },
      [{ role: 'user', content: 'q' }],
      { search: DEFAULT_SEARCH_ASSEMBLY, completion: { timeoutMs: 150 } },
    );
