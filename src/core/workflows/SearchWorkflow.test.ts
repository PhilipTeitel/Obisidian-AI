...
import { SNIPPET_HEADING_OVERHEAD_TOKENS } from '../domain/contextAssembly.js';
import { estimateTokens } from '../domain/tokenEstimator.js';
import type { DocumentNode } from '../domain/types.js';
import type { IEmbeddingPort } from '../ports/IEmbeddingPort.js';
import { DEFAULT_SEARCH_K, mapSearchK, runSearch } from './SearchWorkflow.js';
...
describe('SearchWorkflow', () => {
  ...
  it('B2_snippet_within_budget', async () => {
    ...
    expect(tokens).toBeLessThanOrEqual(
      assembly.totalTokenBudget + SNIPPET_HEADING_OVERHEAD_TOKENS,
    );
  });
});
