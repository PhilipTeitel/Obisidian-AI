...
import type { ChunkNoteResult, DocumentNode, StoredSummary } from '../domain/types.js';
import type { IChatPort } from '../ports/IChatPort.js';
import type { IDocumentStore } from '../ports/IDocumentStore.js';
import { computeDirtyNodeIds, summarizeNote } from './SummaryWorkflow.js';

function node(p: Partial<DocumentNode> & Pick<DocumentNode, 'id' | 'noteId' | 'type'>): DocumentNode {
  return {
    parentId: null,
    headingTrail: [],
    depth: 0,
...
describe('SummaryWorkflow', () => {
  ...
  it('B2_post_order_depth_chain', async () => {
    ...
    expect(upsertSpy.mock.calls.map((c) => c[0])).toEqual([
      'nid-sub',
      'nid-topic',
      'nid-note',
    ]);
