 ...
 * or adapter layers (plugin / sidecar). Same rules for tests under tests/core.
 */
...
/** Core tests use @src/* — must not bridge into plugin/sidecar trees. */
const forbiddenTestCoreAtSrcBridge = /from\s+['"]@src\/(plugin|sidecar)/;

function verifyCoreTree(dir, { isTestCore }) {
  for (const file of walkTsFiles(dir)) {
    ...
    if (isTestCore && forbiddenTestCoreAtSrcBridge.test(text)) {
      fail(`forbidden @src/plugin|@src/sidecar import in ${rel}`);
    }
  }
}

verifyCoreTree(path.join(root, 'src/core'), { isTestCore: false });
verifyCoreTree(path.join(root, 'tests/core'), { isTestCore: true });
