...
const storeSourcePath = fileURLToPath(
  new URL('../../../src/sidecar/adapters/SqliteDocumentStore.ts', import.meta.url),
);
...
describe('SqliteDocumentStore', () => {
  ...
  it('A1_tags_xrefs_replace', async () => {
    ...
    const xref = db.prepare('SELECT target_path FROM cross_refs').all() as {
      target_path: string;
    }[];
  ...
  it('Y2_adapter_path', () => {
    expect(import.meta.url).toContain('/tests/sidecar/adapters/');
  });
});
