...
class FakeJobSteps implements IJobStepPort {
  ...
  transitionTos: import('@src/core/domain/types.js').IndexStep[] = [];
...
class FakeStore implements IDocumentStore {
  ...
  noteMeta: import('@src/core/domain/types.js').NoteMeta[] = [];
  tags: ParsedTag[] = [];
  ...
  async upsertNoteMeta(meta: import('@src/core/domain/types.js').NoteMeta): Promise<void> {
...
): import('@src/core/workflows/IndexWorkflow.js').IndexWorkflowDeps {
  return {
    queue,
    store,
    embed,
...
describe('IndexWorkflow', () => {
  ...
  it('C2_chunker_inputs', async () => {
    ...
    await processOneJob(
      baseDeps(store, jobSteps, queue, fakeEmbed(4)),
      {},
      {
        id: 'q',
        payload,
  ...
  it('D1_skip_content_embed', async () => {
    ...
    expect(contentCalls.every((c) => !c[0].some((t) => t.includes('Skip me')))).toBe(true);
