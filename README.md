...
| ID                                                                                      | Status      | Story                                                                | Size | Notes                                                                                                                                             |
| --------------------------------------------------------------------------------------- | ----------- | -------------------------------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| [VEC-0](docs/features/VEC-0-spike-wa-sqlite-sqlite-vec-obsidian-electron-bundle.md)     | Done        | Spike: sqlite-vec semantics (Node); WASM deferred to VEC-2            | M    | ADR-001 + `npm run spike:vec0` (default DB under `~/.obsidian-ai/`). In-Obsidian WASM in VEC-2.                                                       |
| [VEC-1](docs/features/VEC-1-per-vault-db-path-resolver-and-settings.md)                 | Done | Per-vault DB path resolver + settings (`vectorStoreAbsolutePath`)    | M    | `os.homedir()/.obsidian-ai/`, vault name sanitization, privacy (no cross-vault path). Plan Phase 1.                                               |
