...
| `RuntimeErrorDomain`    | `"provider"                                                                    | "network"                                                                                                      |
...
| [HIER-3](docs/features/HIER-3-implement-wikilink-parser-for-cross-reference-extraction.md)                 | Done   | Implement wikilink parser for cross-reference extraction               | S    | New file: `src/utils/wikilinkParser.ts`. Extract `[[target]]` and `[[target                                                                                                                                                                                                                                                    |
...
### Epic 19: Native SQLite + sqlite-vec Store (prompt 05)

Implement [docs/prompts/05-SQLITE-vector-store-implementation.md](docs/prompts/05-SQLITE-vector-store-implementation.md): file-backed hierarchical index under the user profile (per-vault `vector-store.<vaultName>.sqlite3`), optional per-vault absolute path, lazy DB open, real migration execution, sqlite-vec vector search, removal of the flat `LocalVectorStoreRepository` pipeline and `saveData` index payloads. **Engineering plan:** [docs/plans/sqlite-vector-store-implementation-plan.md](docs/plans/sqlite-vector-store-implementation-plan.md).


| ID                                                                                      | Status      | Story                                                                | Size | Notes                                                                                                                                             |
| --------------------------------------------------------------------------------------- | ----------- | -------------------------------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| [VEC-0](docs/features/VEC-0-spike-wa-sqlite-sqlite-vec-obsidian-electron-bundle.md)     | Done        | Spike: wa-SQLite + sqlite-vec in Obsidian/Electron bundle            | M    | ADR-001 + `npm run spike:vec0` (Node proof). Obsidian WASM wiring in VEC-2. Blocks VEC-2–4 were satisfied for stack choice.                       |
| [VEC-1](docs/features/VEC-1-per-vault-db-path-resolver-and-settings.md)                 | Not Started | Per-vault DB path resolver + settings (`vectorStoreAbsolutePath`)    | M    | `os.homedir()/.obsidian-ai/`, vault name sanitization, privacy (no cross-vault path). Plan Phase 1.                                               |
| [VEC-2](docs/features/VEC-2-lazy-db-lifecycle-open-create-dispose.md)                   | Not Started | Lazy DB lifecycle: open/create, dispose, sqlite-vec load             | M    | First use of hierarchical store; create parent dir; close on unload. Plan Phase 2.                                                                |
| [VEC-3](docs/features/VEC-3-run-vector-store-migrations-on-real-connection.md)          | Not Started | Run `VECTOR_STORE_MIGRATIONS` against real connection                | M    | `metadata` applied-migration tracking; idempotent startup. Plan Phase 3.                                                                          |
| [VEC-4](docs/features/VEC-4-reimplement-sqlitevecrepository-with-sql-and-sqlite-vec.md) | Not Started | Reimplement `SqliteVecRepository` with SQL + vec (drop JSON persist) | XL   | Full `HierarchicalStoreContract`; transactions; remove `hierarchicalStore` from `data.json`. Plan Phase 4.                                        |
| [VEC-5](docs/features/VEC-5-remove-flat-store-hierarchical-only-search.md)              | Not Started | Remove flat store; hierarchical-only search in UI and services       | L    | Drop `LocalVectorStoreRepository` from bootstrap; strip `IndexingService` / `SearchService` / `main.ts` / `ChatService` flat paths. Plan Phase 5. |
| [VEC-6](docs/features/VEC-6-tooling-and-user-documentation.md)                          | Not Started | Tooling + user docs: query script, IDE note, sync/uninstall/privacy  | M    | `scripts/query-store.mjs` + sqlite-vec; README: reindex, `~/.obsidian-ai/`, per-vault files, cloud sync warning. Plan Phase 6.                    |
