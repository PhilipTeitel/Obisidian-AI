...
- [Obsidian AI Plugin — Iteration 4](#obsidian-ai-plugin--iteration-4)
  ...
  - [Key Design Decisions](#key-design-decisions)
    ...
    - [24. Deterministic Agentic Note Synthesis](#24-deterministic-agentic-note-synthesis)
  ...
  - [Backlog Items](#backlog-items)
    ...
    - [Epic 12: Deterministic agentic note synthesis (REQ-007)](#epic-12-deterministic-agentic-note-synthesis-req-007)
  - [License](#license)
...
- [docs/requirements/REQ-006-bug-001-chat-accuracy-ux-search.md](docs/requirements/REQ-006-bug-001-chat-accuracy-ux-search.md) — BUG-001: chat source accuracy, 45ble messages, vault-organization time queries, and search-safe prompts (refined from [docs/requests/BUG-001.md](docs/requests/BUG-001.md))
- [docs/requirements/REQ-007-deterministic-agentic-note-synthesis.md](docs/requirements/REQ-007-deterministic-agentic-note-synthesis.md) — Deterministic pre-query planning, bounded note tools, topic synthesis drafts, and logging for retrieval plans/token usage.
...
- [docs/decisions/ADR-018-deterministic-agentic-note-synthesis.md](docs/decisions/ADR-018-deterministic-agentic-note-synthesis.md) — Accepted bounded agent loop: pre-query retrieval plan, stable source set, draft-only synthesis, Ollama-first reasoning, fixed testing budgets, and usage logging.
...
sequenceDiagram
    ...
    participant SQLite as SQLite + sqlite-vec
    participant Tools as Bounded Note Tools
    ...
    alt Usable grounded context found
        ...
        Sidecar->>Sidecar: Log warn with exceeded budget
        Sidecar-->>Transport: Terminal bounded-run response
    end
...
| Pre-query reasoning | Ollama first, OpenAI next               | REQ-007 prioritizes local Ollama reasoning before OpenAI; future providers remain additive behind ports |
...
For deterministic pre-query reasoning ([REQ-007](docs/requirements/REQ-007-deterministic-agentic-note-synthesis.md), [ADR-018](docs/decisions/ADR-018-deterministic-agentic-note-synthesis.md)), Ollama is the first implementation target. OpenAI follows next, and later providers remain additive behind the same hexagonal provider boundary.
...
File operations are a later capability, not part of the first deterministic synthesis iteration:

- **First REQ-007 slice:** generated output is returned as draft content in chat/workflow results only; the plugin does not write generated notes or edits to the vault.
- **Future file-writing slice:** saving generated output must use the existing configurable allowed output folders, distinct from indexed-folder rules.
- **No review UI requirement:** because file writing is deferred, the first iteration does not need a proposed-edit review UI.
- **Write boundary:** when implemented later, file operations remain plugin-side via the Obsidian API, not sidecar direct filesystem writes.
...
### 24. Deterministic Agentic Note Synthesis

> **ADR:** [ADR-018 — Deterministic agentic note synthesis loop](docs/decisions/ADR-018-deterministic-agentic-note-synthesis.md) (Accepted). Traces to [REQ-007](docs/requirements/REQ-007-deterministic-agentic-note-synthesis.md).

`ChatWorkflow` moves from single-shot RAG to a bounded synthesis loop:

```text
interpret prompt -> retrieval plan -> bounded note tools -> grounded answer or draft
```

The workflow first creates a structured retrieval plan before issuing vector or keyword queries. The plan includes the user task, topic/entities, effective filters, date scope, requested output type, and planned note-tool calls. When a date-bounded synthesis request omits a range, the default date range is one week. Vault-specific conventions come from the existing `vaultOrganizationPrompt` setting.

The first tool surface is intentionally narrow:

- `search_notes` delegates to `SearchWorkflow` and preserves hybrid/path/date/tag filtering.
- `read_note` retrieves note/node context needed after a search hit or cross-reference.
- daily-note/path/date scoped discovery reuses the existing daily-note settings and date filters.
- draft assembly returns synthesized content without writing files to the vault.

Determinism is measured in tiers:

- Same prompt/settings/model/vault state must produce the same retrieval plan.
- Source set must be stable except documented ranking ties.
- Draft structure must repeat for the requested output type.
- Prose may vary only if facts and source grounding remain equivalent.
...
│   │   │   ├── dateRangeResolver.ts         # NL time phrases → dateRange (ADR-016)
│   │   │   ├── agentRetrievalPlan.ts         # Structured plan for pre-query synthesis (ADR-018)
│   │   │   ├── agentBudgets.ts               # Fixed test-oriented agent limits (ADR-018)
...
│   │       ├── ChatWorkflow.ts              # Agentic chat / synthesis orchestration
│   │       ├── AgentToolWorkflow.ts          # Bounded note search/read/draft tool execution (ADR-018)
...
- **Agentic synthesis (ADR-018):** For synthesis/reporting prompts, chat output may be a draft note-style response. Prompt-requested formats are honored when feasible; otherwise bullet lists are the default structure. The first REQ-007 slice returns draft content only and does not write generated files to the vault.
...
| `IAgentToolPort`   | `execute`              | `(call: AgentToolCall) → Promise<AgentToolResult>`                                                                                                                          | Bounded note tools for search/read/draft assembly; file-write tools deferred ([ADR-018](docs/decisions/ADR-018-deterministic-agentic-note-synthesis.md)) |
...
| `vaultOrganizationPrompt` | `string`                                        | `''`                          | User-supplied description of vault conventions (daily notes, tags, folders, wikilinks, people/project conventions) used to help the assistant target retrieval ([ADR-011](docs/decisions/ADR-011-vault-only-chat-grounding.md), [ADR-018](docs/decisions/ADR-018-deterministic-agentic-note-synthesis.md)) |
...
| `agentOutputFolders`   | `string[]`                                         | `['AI-Generated']`            | Allowed folders for future agent-created notes; not used by the first REQ-007 draft-only slice |
| `maxGeneratedNoteSize` | `number`                                           | `5000`                        | Future max characters for agent-generated notes; not used for vault writes in the first REQ-007 slice |
...
REQ-007 does not extend or consume the existing write path in its first slice. Deterministic synthesis returns draft content only; future file-writing work must re-enter design/story planning before using `agentOutputFolders`.
...
Move chat from single-shot RAG to deterministic pre-query planning, bounded note tools, and grounded draft synthesis. This epic implements [REQ-007](docs/requirements/REQ-007-deterministic-agentic-note-synthesis.md) and [ADR-018](docs/decisions/ADR-018-deterministic-agentic-note-synthesis.md). The first slice is draft-only: generated content is returned in chat/workflow output and is not written to the vault.

| ID    | Status      | Story                                                                 | Size | Notes |
| ----- | ----------- | --------------------------------------------------------------------- | ---- | ----- |
| [AGT-2](docs/features/AGT-2-retrieval-plan-contract.md) | Not Started | Retrieval plan contract and deterministic plan tests                  | M    | Define `RetrievalPlan`; same prompt/settings/model/vault state must produce identical plan; default date range one week; vault conventions from `vaultOrganizationPrompt`; covers REQ-007 S1/S2/S7 |
| AGT-3 | Not Started | Bounded note tools for search, note read, and draft assembly          | L    | Tool calls delegate to `SearchWorkflow` / `IDocumentStore`; no vault writes; fixed high testing budgets; covers REQ-007 S3/S4/S9 |
| AGT-4 | Not Started | Agentic `ChatWorkflow` loop: plan → tools → synthesize                | L    | Ollama first for pre-query reasoning; OpenAI next; stable source set except ranking ties; covers REQ-007 S3/S6/S7 |
| AGT-5 | Not Started | Topic synthesis draft output and prompt-requested formats             | M    | Bullet list default; honor requested format where feasible; sources equal contributing notes; covers REQ-007 S4/S5/S6 |
| AGT-6 | Not Started | Agent observability: retrieval plan, tool trace, token usage, budgets | M    | Log retrieval plan, searches, note reads, source set, actual token usage when reported, and `warn` on budget exceedance; covers REQ-007 S8 |
| PRV-3 | Not Started | Ollama planner adapter support for pre-query reasoning                | M    | Provider-specific planner behavior behind hexagonal boundary; prepares OpenAI follow-up without changing core workflow |
