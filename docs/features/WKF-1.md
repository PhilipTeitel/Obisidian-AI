...
| 2 | `src/core/workflows/SummaryWorkflow.test.ts` | Unit tests with fake chat + in-memory store (or typed fakes). |
...
| 4 | `src/sidecar/adapters/SqliteDocumentStore.test.ts` | Cover read methods + column mapping. |
...
3. `src/sidecar/adapters/SqliteDocumentStore.ts` + `.test.ts` — implement reads (covers A1–A2).
4. `src/core/workflows/SummaryWorkflow.ts` — traversal + prompts + skip (covers B\*, C\*).
5. `src/core/workflows/SummaryWorkflow.test.ts` — fakes + assertions (covers B\*, C\*).
...
*Created: 2026-04-05 | Story: WKF-1 | Epic: 4 — Index, summary, and embedding workflows*
