...
| ----------------------- | ------------------------------------------------------------------------------------------------------------ |
| _(none for this story)_ | Tooling only; epic-level ADRs in README still bound the repo (e.g. ADR-006 for native code in sidecar only). |
...
- [x] **A1** — `npm run format:check` (or `prettier --check .`) passes on the repo after formatting is applied once.
...
- [x] **B2** — ESLint `no-restricted-imports` (or stricter) blocks `obsidian` and `better-sqlite3` imports under `src/core/**`.
...
- [x] **C1** — `npm run typecheck` runs `tsc --noEmit` for all TS projects introduced in FND-1 (composite or multiple `-p` flags documented).
...
- [x] **Y1** — **(binding)** `package.json` lists `vitest`, `eslint`, `prettier`, and `typescript` (or `typescript-eslint` meta) as devDependencies with pinned or lockfile-resolved versions committed.
...
- [x] **Z2** — `npm run lint` passes (or only pre-existing warnings documented in PR)
  - Evidence: `npm run lint`

- [x] **Z3** — No `any` types in any new or modified file for this story (configs exempt if unavoidable — prefer typed eslint config)
  - Evidence: PR diff review + optional `@typescript-eslint/no-explicit-any`

- [x] **Z4** — **N/A** — No `@shared/types` alias; imports remain relative or path-mapped per repo convention only if added in FND-3+.

- [x] **Z5** — **N/A** — No new application logging; tooling only. Optional: eslint rule for `no-console` in `src/core` deferred to later stories.
...
_Created: 2026-04-04 | Story: FND-2 | Epic: 1 — Scaffold, toolchain, and domain contracts_
