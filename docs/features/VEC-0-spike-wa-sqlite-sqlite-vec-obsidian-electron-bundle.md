...
**Plan**: [docs/plans/sqlite-vector-store-implementation-plan.md](../plans/sqlite-vector-store-implementation.md) — Phase 0
...
- [ ] **A1** — Written decision record links to prompt 05 and implementation plan Phase 0
- [ ] **A2** — Document lists minimum Obsidian/Electron versions validated (or “not yet validated” with follow-up)
- [ ] **A3** — Document states mobile scope: **out of scope** for MVP per prompt 05 §10 unless explicitly expanded

### Phase B: Technical proof

- [ ] **B1** — Proof opens/creates a `.sqlite3` file at a **non-vault** absolute path
- [ ] **B2** — Proof loads **sqlite-vec** and creates a `vec0` table compatible with hierarchical `node_embeddings` intent (node id, embedding type, float vector of fixed dimension matching [vectorStoreSchema.ts](../../src/storage/vectorStoreSchema.ts) or documented deviation)
- [ ] **B3** — Proof runs at least one **vector search** API supported by sqlite-vec (KNN / ANN as per extension docs), not pure JS cosine over the full table
- [ ] **B4** — Proof closes DB cleanly (no handle leak in long-running dev scenario)

### Phase Z: Quality gates

- [ ] **Z1** — `npm run build` still passes after any spike deps/config land (or spike is isolated branch with documented merge steps)
- [ ] **Z2** — No production user-facing behavior change required for spike-only merge (optional: spike lives on branch until VEC-1)
