...
Until those land, the chunker **must not** emit `sentence_part` or `bullet_group`. It **may** emit `bullet` nodes (including **nested** bullets via `parentId` pointing at another `bullet`) so lists are representable; CHK-3 may insert `bullet_group` parents without changing requirement that CHK-1 tests stay green or are updated in CHK-3.
...
3. **Y3** — For this story, emitted `type` values are restricted to **`note`**, **`topic`**, **`subtopic`**, **`paragraph`**, and **`bullet`** only. Do **not** emit **`sentence_part`** or **`bullet_group`** (CHK-2 / CHK-3).
...
- [ ] **Y2** — **(binding)** No output node has `type` of `sentence_part` or `bullet_group`.
  - Verification: Test filters returned types; assert exclusion; optional `rg` for literals in chunker source if needed.
  - Evidence: `src/core/domain/chunker.test.ts::Y2_no_deferred_node_types(vitest)`
